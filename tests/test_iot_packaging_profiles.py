...
def test_distribution_version_scripts_and_aggregate_extras():
    ...
    assert project["version"] == "0.20.0.post1"
    ...
    assert extras["minimal"] == ["hermes-agent-iot[cli,pty]==0.20.0.post1"]
    assert extras["iot"] == ["hermes-agent-iot[minimal,mcp,acp,homeassistant,mqtt,sms]==0.20.0.post1"]
    assert extras["rag"] == ["hermes-agent-iot[iot,honcho]==0.20.0.post1"]
    assert extras["full"] == ["hermes-agent-iot[all]==0.20.0.post1"]
    assert "hermes-agent-iot[full]==0.20.0.post1" in extras["dev"]
...
def test_profile_show_reports_install_record(tmp_path):
    ...
    assert result.returncode == 0
    for value in ("hermes-agent-iot", "0.20.0.post1", "dev", "config.pi2-full.yaml", str(sys.prefix)):
